...
import 'package:scribble_clone/sidebar/player_drawer.dart';
import 'package:scribble_clone/waiting_lobby_screen.dart';
...
class PaintScreen extends StatefulWidget {
  ...
  const PaintScreen({required this.data, required this.screenFrom, Key? key})
      : super(key: key);
...
class _PaintScreenState extends State<PaintScreen> {
  ...
  int maxPoints = 0;
  String winner = '';
  bool isShownFinalLeaderboard = false;
  ...
  void connect() {
    _socket = IO.io('http://192.168.101.6:3000', <String, dynamic>{
      'transports': ['websocket'],
      'autoConnect': false
    });
    _socket.connect();

    if (widget.screenFrom == 'create') {
                            ...
                            Row(
                              children: [
                                ...
                                IconButton(
                                  onPressed: () {
                                    _socket.emit(
                                        'clear-screen', dataOfRoom['roomname']);
                                  },
